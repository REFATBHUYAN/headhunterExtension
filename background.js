const BACKEND_URL = "https://bloomix-api.onrender.com/v1"
...
async function processUrlSequentially(searchId, url) {
  try {
    ...
    await sendToBackend(
      ...
      "/api/headhunter/process-linkedin-dom",
    ).catch(e => console.error("SendToBackend failed", e))
...
async function handleExtractionComplete(request, tabId) {
  try {
    ...
    await markUrlProcessedAndContinue(searchId, request.profileUrl || request.url)
  }
}
...
// // Process individual URL sequentially
// async function processUrlSequentially2(searchId, url) {
//   try {
//     const searches = new Map(Object.entries(await getStorage(SEARCHES_STORAGE_KEY)))
//     const searchSession = searches.get(searchId)
...
//     // ❌ removed markUrlProcessedAndContinue here
//   }
// }


// // Handle extraction completion
// async function handleExtractionComplete2(request, tabId) {
//   try {
//     const { searchId, profileUrl, url } = request
//     const finalUrl = profileUrl || url
...
//     console.error("❌ Error handling extraction completion:", error)
//   }finally {
//   if (finalUrl) await markUrlProcessedAndContinue(searchId, finalUrl)
// }
// }
// // Handle extraction completion
// async function handleExtractionComplete(request, tabId) {
